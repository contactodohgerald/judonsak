                ...
                <tbody>
                    @foreach ($logs as $log)
                        <tr>
                            ...
                            <td>
                                <a href="{{ route('profile.show', ['[profile]' => $log->person->slug]) }}">
                                    {{ $log->person->first_name . ' ' . $log->person->last_name ?? 'No Name Set' }}
                                </a>
                            </td>
                            <td>
                                {{ $log->logable['name'] }}
                            ...
                            <td>
                                <a style="cursor: pointer; font-size: 20px">
                                    <span class="badge bg-info">{{ appLogAction($log->action_type) }}</span>
                                </a>
                            </td>
                        </tr>
                    @endforeach
